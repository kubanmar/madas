...
def test_calculate(material):

    prop = {"test" : {"path": 1}}
    material.set_data(prop)

    fp = Fingerprint("PROP", property_path = "test/path").calculate(material)
    ...
    fp2 = Fingerprint("PROP", property_path = "test/path").calculate(material)
...
def test_similarity(material):
    ...
    fp = Fingerprint("PROP", property_path = "test/path", pass_on_exceptions=False).calculate(material)

    prop = {"test1" : {"path": 2}}
    material.set_data(prop)

    fp2 = Fingerprint("PROP", property_path = "test1/path").calculate(material)

...
for index in range(nentries):
    ...
    with multiprocessing.Pool() as p:
        ...
        xs = np.append(xs,np.array(p.map(dos_fp.get_similarity,[dos_fps[x] for x in range(index+1,nentries)])))
        ys = np.append(ys,np.array(p.map(sym_fp.get_similarity,[sym_fps[x] for x in range(index+1,nentries)])))
